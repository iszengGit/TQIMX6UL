...
static int tsc_get_xy(int irq, void *dev_id, int *px,int *py)
{
	...
	int x, y;
	int start;
	int sum_x,sum_y, min_x,min_y, max_x,max_y;
	int count,i;
	...
	if(i>=3){
		*px = (sum_x-max_x-min_x)/(count-2);
		*py = (sum_y-max_y-min_y)/(count-2);
	}
	else
	{
		*px = sum_x/count;
...
static irqreturn_t tsc_irq_fn_tq(int irq, void *dev_id)
{
	struct imx6ul_tsc *tsc = dev_id;
	int x,y;
	int ret;
	ret = tsc_get_xy(irq, dev_id,&x,&y);
...
static int imx6ul_tsc_probe(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev,"start imx6ul_tsc ...\n");
	int check_gpio = of_get_named_gpio(np, "check-gpio", 0);
	if (gpio_is_valid(check_gpio)) {
		err = devm_gpio_request(&pdev->dev, check_gpio, "check gpio");
                if (err) {
                        dev_err(&pdev->dev, "use Capacitance_ts\n");
                        return err;
                }
	}
	//return 0;
	...
	input_dev->name = "Resistive_ts";//"iMX6UL TouchScreen Controller";
	...
	err = devm_request_threaded_irq(tsc->dev, tsc_irq,
					NULL, tsc_irq_fn_tq, IRQF_ONESHOT,
...
static int __init module_tsc_init(void)
{
        return platform_driver_register(&imx6ul_tsc_driver);
}
late_initcall(module_tsc_init);
