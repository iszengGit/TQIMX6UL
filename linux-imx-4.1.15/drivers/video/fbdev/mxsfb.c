...
static void dump_clks(struct clk *clk)
{
	int i;
	struct clk *parent;
	//printk("xxxxxxxxxxxxxxxxx\n");
	//printk("%s(%ld)", __clk_get_name(clk), clk_get_rate(clk));
	for (i = 0; i<5; ++i)
	{
		...
		//printk("<-%s(%ld)", __clk_get_name(parent), clk_get_rate(parent));
		clk = parent;
	}
	//printk("\nxxxxxxxxxxxxxxxxx\n");
}
...
static void mxsfb_enable_controller(struct fb_info *fb_info)
{
	...
	log_d("fb_info->var.pixclock = %d %d\n" ,fb_info->var.pixclock, __LINE__);
	...
	log_d("fb_info->var.pixclock = %d\n" ,fb_info->var.pixclock);
...
//extern int tq_set_display(struct display_timing *dt);
...
static int mxsfb_init_fbinfo(struct mxsfb_info *host)
{
	...
	if (ret){
		//dev_err(dev, "mxsfb_init_fbinfo_dt failed\n");
		return ret;
	}	
...
static int mxsfb_probe(struct platform_device *pdev)
{
	...
	dump_clks(host->clk_pix);
	dump_clks(host->clk_axi);
	dump_clks(host->clk_disp_axi);
	return 0;
