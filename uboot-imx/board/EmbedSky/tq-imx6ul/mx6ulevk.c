...
#define UART5_TXD_GPIO	IMX_GPIO_NR(1, 30) //JK Add
#define UART5_RXD_GPIO	IMX_GPIO_NR(1, 31) //JK Add
...
static void setup_gpmi_nand(void)
{
    ...
    if ((CPU_CORE_FREQ<=450000000) && (CPU_CORE_FREQ>350000000))
	    setup_gpmi_io_clk((3 << MXC_CCM_CSCDR1_BCH_PODF_OFFSET) | \
			  (3 << MXC_CCM_CSCDR1_GPMI_PODF_OFFSET)); 		// 396M/4 == 100M
    else if ((CPU_CORE_FREQ<=550000000) && (CPU_CORE_FREQ>450000000))
	    setup_gpmi_io_clk((4 << MXC_CCM_CSCDR1_BCH_PODF_OFFSET) | \
			  (4 << MXC_CCM_CSCDR1_GPMI_PODF_OFFSET)); 		// 528M/5 == 100M
...
void logoDisplay(void){
	run_command("run loadlogo",0);
	//run_command("nand read ${splashimage} logo", 0);
	//run_command("bmp display ${splashimage} m m", 0);
}

static iomux_v3_cfg_t const test_pads[] = {
	MX6_PAD_JTAG_MOD__GPIO1_IO10 | MUX_PAD_CTRL(NO_PAD_CTRL),
};
...
void gpio_test()
{
	...
	imx_iomux_v3_setup_multiple_pads(test_pads, ARRAY_SIZE(test_pads));
    for(i=0;i<10;++i){
        printf("test for %d\n",i);
...
struct display_info_t displays[] = {
	...
	//for embedsky.ini setting
	{
		...
		.enable	= do_enable_parallel_lcd,
		.mode	= {
			.name			= "panel_lcd", //used at arch/arm/imx-common/video.c
		}
	 },
};
